...
function writeStorage(prefs: A11yPrefs) {
  try {
    ...
    // quota exceeded or private mode — keep the in-memory state only
  }
}
...
export function useA11yPrefs(): A11yPrefs {
  return useSyncExternalStore(a11yStore.subscribe, a11yStore.getSnapshot, a11yStore.getServerSnapshot);
}
