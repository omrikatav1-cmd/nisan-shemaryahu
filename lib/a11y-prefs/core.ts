// Pure helpers, no "use client" — safe to import from the root layout
// (Server Component) for the FOUC bootstrap <script>.
...
export const A11Y_STORAGE_KEY = "nisan_a11y_prefs_v1";
...
// the FOUC bootstrap script below, so the two can never drift apart.
const CLASS_RULES: ReadonlyArray<[className: string, predicate: (p: A11yPrefs) => boolean, js: string]> = [
...
// Inline <script> executed in <head> before React hydrates, so persisted
// prefs apply synchronously and the user never sees a flash of default styling.
